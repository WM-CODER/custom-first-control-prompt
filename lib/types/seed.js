 ...
 */
import { createUserMessage, deepFreeze, MessageId } from '@deepseek-ai/dsh-llm';
 ...
 */
export const TRANSCRIPT_RESERVED_TAGS = [
    '<custom-history',
    '</custom-history>',
    '<exchange>',
    '</exchange>',
 ...
 */
export function renderTranscript(pairs) {
    const exchanges = pairs.map(pair => `<exchange>\n<user>${pair.user}</user>\n<assistant>${pair.assistant}</assistant>\n</exchange>`);
 ...
 */
export function seededMessageSource(event) {
 ...
 */
export function isSeededByPlugin(source) {
 ...
 */
export function hasSeededHistory(session) {
 ...
 */
export function buildHistoryMessage(pairs) {
 ...
 * @param pairs - ordered reference exchanges.
 */
export function seedTranscript(session, pairs) {
 ...
 */
export function buildSeedEvents(pairs, startSeq = 0, startTurn = 1, time = Date.now()) {
    const events = [];
    let seq = startSeq;
    pairs.forEach((pair, index) => {
        ...
        events.push({ type: 'turn/start', seq: seq++, time, data: { turn } }, { type: 'step/start', seq: seq++, time, data: { turn, step } }, { type: 'user/message', seq: seq++, time, data: user, surfaceOp: 'append' }, {
            type: 'assistant/message',
            seq: seq++,
            time,
            surfaceOp: 'append',
            data: assistantData,
        }, { type: 'step/end', seq: seq++, time, data: { turn, step } }, { type: 'turn/end', seq: seq++, time, data: { turn, reason: { kind: 'completed' } } });
    });
    return events;
}
/**
 ...
 * @param pairs - ordered reference exchanges.
 */
export function appendSeedTurns(session, pairs) {
    pairs.forEach((pair, index) => {
        const turn = index + 1;
        const step = 1;
 ...
 */
export function buildSeedMessages(pairs) {
    const messages = [];
