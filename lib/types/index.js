...
import { appendSeedTurns, buildHistoryMessage, buildSeedEvents, buildSeedMessages, hasSeededHistory, isSeededByPlugin, seededMessageSource, seedTranscript, TRANSCRIPT_RESERVED_TAGS } from "./seed.js";
    ...
    if (seedMode === 'intercept') {
        ...
        ctx.on('llm/stream', (options, next) => {
            ...
            if (config.includeSubagents !== true
                && ctx.sessions.get(sessionId)?.header.origin === 'subagent') {
                return next();
            }
            const cloned = { ...options, messages: [...seedMessages, ...options.messages] };
            reentry.add(cloned);
